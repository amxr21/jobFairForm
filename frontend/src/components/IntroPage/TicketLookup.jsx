...
const TicketLookup = ({ open, onClose }) => {
    ...
    const lookup = async () => {
        ...
        try {
            ...
            setError(err.response?.data?.error || t("ticketLookup.genericError"));
        } finally {
            setLoading(false);
        }
    };
                ...
                {!applicant ? (
                    <>
                        <div className="bg-[#0E7F41] p-4 md:p-6 text-center">
                            ...
                            <h1 className="text-white text-base md:text-lg font-bold">{t("nav.checkMyTicket")}</h1>
                            <p className="text-white/80 text-[11px] md:text-xs mt-1">{t("ticketLookup.enterUniversityId")}</p>
                        </div>
                        <div className="bg-white dark:bg-[#131b2c] p-4 md:p-6 flex flex-col gap-2.5 md:gap-3">
                            {/* University ID is a numeric code, not free text —
                                dir="ltr" so it doesn't reorder under an
                                Arabic-direction form, matching the same field
                                in Input.jsx (forceLtr there). */}
                            <input
                                ref={inputRef}
                                dir="ltr"
                                value={uniId}
                                ...
                                placeholder={t("ticketLookup.idPlaceholder")}
                                inputMode="numeric"
                            ...
                            >
                                ...
                                {loading ? t("ticketLookup.lookingUp") : t("ticketLookup.getMyTicket")}
                            </button>
                        </div>
                    </>
                ) : (
                    <>
                        ...
                        <div className="bg-white dark:bg-[#131b2c] p-4 md:p-6 flex flex-col items-center gap-3 md:gap-4">
                            ...
                            <div className={`w-full text-center text-[11px] md:text-xs font-medium rounded-md px-2.5 md:px-3 py-1.5 md:py-2 ${applicant.attended ? "bg-green-50 text-green-700 dark:bg-green-500/15 dark:text-green-300" : "bg-amber-50 text-amber-700 dark:bg-amber-500/15 dark:text-amber-300"}`}>
                                {applicant.attended ? t("ticketLookup.checkedIn") : t("ticketLookup.showAtEntrance")}
                            </div>
                            <button onClick={reset} className="text-[11px] md:text-xs text-fg-faint hover:text-fg-muted">
                                {t("ticketLookup.lookupDifferentId")}
                            </button>
                        </div>
                    </>
                )}
