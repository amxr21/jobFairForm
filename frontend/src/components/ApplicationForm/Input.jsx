...
const INPUT_CLASSES = "h-8 md:h-9 w-full bg-surface-field border rounded-md py-1 px-2 text-xs md:text-sm focus:outline-none focus:ring-2 focus:border-transparent transition-all duration-200";
...
const Input = ({ label, type, name, fieldClasses = '' }) => {
        ...
        return (
            <div className={`${WRAPPER_CLASSES} h-full ${fieldClasses}`}>
                ...
                <textarea
                    ...
                    className={`flex-1 w-full bg-surface-field border rounded-md py-1.5 px-2 text-sm resize-none min-h-0 focus:outline-none focus:ring-2 focus:border-transparent transition-all duration-200 ${getBorderClass()}`}
                />
                {renderError()}
            </div>
        );
    ...
    if (config.hasPrefix) {
        return (
            <div className={`${WRAPPER_CLASSES} ${fieldClasses}`}>
                {renderLabel()}
                <div className={`flex items-center h-8 md:h-9 w-full bg-surface-field border rounded-md focus-within:ring-2 focus-within:border-transparent transition-all duration-200 ${getBorderClass()}`}>
