...
//
// Titles are message KEYS here, not literal strings — STEPS is a
// module-level const so it can't call the t() hook itself; the component
// below resolves each key through t() at render time. Step 3's "(Optional)"
// is appended separately via steps.optionalSuffix rather than baked into the
// title string, so the Arabic translation isn't stuck re-deriving a
// grammatically awkward parenthetical.
const STEPS = [
    { Icon: User, titleKey: "steps.personalInfo" },
    { Icon: GraduationCap, titleKey: "steps.professionalInfo" },
    { Icon: Sparkles, titleKey: "steps.preferences", optional: true },
];
...
const ProgressSection = ({ currentStep }) => {
    ...
    const { Icon, titleKey, optional } = STEPS[currentStep - 1] || STEPS[0];
    const title = t(titleKey) + (optional ? t("steps.optionalSuffix") : "");
        ...
        <div className="relative flex flex-row md:flex-col items-center md:items-stretch gap-3 md:gap-8 w-full border py-2.5 px-4 md:py-10 md:px-12 min-h-0 md:min-h-[28rem] h-fit md:h-full bg-[#0E7F41] rounded-2xl md:rounded-s-[4em] md:rounded-e-3xl overflow-hidden">
            ...
            {currentStep === 3 && (
                <p className="hidden md:block text-white/80 text-xs md:text-sm">
                    {t("steps.preferencesOptionalNote")}
                </p>
            )}
